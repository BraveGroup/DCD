...
def get_label_anno(label_path):
    ...
    if not os.path.isfile(label_path): #no label in waymo
        lines=[]
    else:
    ...
    annotations['name'] = np.array([x[0].capitalize() for x in content])
...
#             elif len(f)==11:
#                 prog = re.compile(r'^\d{7}.txt$')
#             else:
#                 print('ERROR not legal txt\n')
#             break
...
def get_label_annos(label_folder, image_ids=None):
    ...
    if image_ids is None:
        for file in os.listdir(label_folder):
            ...
            annos.append(get_label_anno(path))
    else:
        for idx in image_ids:
            path = os.path.join(label_folder,idx+'.txt')
            annos.append(get_label_anno(path))
    return annos

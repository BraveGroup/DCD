...
class KITTIDataset(Dataset):
	def __init__(self, cfg, root, is_train=True, transforms=None, augment=True):
		...
		self.root = root
		self.infer_right_img=cfg.DATASETS.INFER_ON_RIGHT_IMG
		...
		self.num_classes = cfg.DATASETS.MAX_CLASSES_NUM
		...
		self.multi_scale= (self.is_train and len(cfg.INPUT.AUG_PARAMS)>1 ) \
			or (not self.is_train and len(cfg.DATASETS.TTA_AUG_PARAMS)>1 )

		if self.is_train and augment:
			self.augmentation = get_composed_augmentations()
		elif self.use_tta:
			self.augmentation = get_composed_augmentations(self.tta_aug_params)
		else:
		...
		self.logger = logging.getLogger("DGDE.dataset")
		self.logger.info("Initializing KITTI {} set with {} files loaded.".format(self.split, self.num_samples))

		#load ann img id
		if self.split=='val':
			self.kpts_ann=json.load(open('kpts_ann/kpts_ann_val.json'.format(self.extra_kpts_num),'r'))
		elif self.split=='train':
			self.kpts_ann=json.load(open('kpts_ann/kpts_ann_train.json'.format(self.extra_kpts_num),'r'))
		else: ## trainval, test
			dict1=json.load(open('kpts_ann/kpts_ann_val.json'.format(self.extra_kpts_num),'r'))
			dict2=json.load(open('kpts_ann/kpts_ann_train.json'.format(self.extra_kpts_num),'r'))
			dict1.update(dict2)
			self.kpts_ann=dict1
		...
		if self.is_train:
			...
			for idx in tqdm(range(len(image_files))):
				...
				if len(objs)>0:
					new_image_files.append(self.image_files[idx])
					new_label_files.append(self.label_files[idx])
	...
	def get_label_objects(self, idx):
		...
		single_kpts_anns=self.kpts_ann[str(int(label_filename[-10:-4]))]
		return read_label(label_filename,single_kpts_anns,self.extra_kpts_num)
	...
	def __getitem__(self, idx):
		...
		if idx >= self.num_samples or self.infer_right_img:
		...
		img_w, img_h = img.size
		if self.multi_scale:
			pad_size = np.zeros(2,dtype=np.int64)
		else:
			img, pad_size = self.pad_image(img)
		final_img_w,final_img_h=img.size
		final_output_w,final_output_h=final_img_w//self.down_ratio , final_img_h//self.down_ratio
		...
		if self.enable_edge_fusion:
			...
			max_edge_length = (final_img_w//self.down_ratio + final_img_h//self.down_ratio) * 2
			input_edge_indices = np.zeros([max_edge_length, 2], dtype=np.int64)
		...
		if self.split == 'test':
			...
			if self.enable_edge_fusion:
				...
				target.add_field('final_output_w',final_output_w)
				target.add_field('final_output_h',final_output_h)
		...
		# heatmap
		heat_map = np.zeros([self.num_classes, final_output_h, final_output_w], dtype=np.float32)
		ellip_heat_map = np.zeros([self.num_classes, final_output_h, final_output_w], dtype=np.float32)
		# classification
		...
		EXTRA_KPTS_3D=np.zeros([self.max_objs, self.extra_kpts_num+10, 3], dtype=np.float32)
		EXTRA_KPTS_2D=np.zeros([self.max_objs, self.extra_kpts_num+10, 3], dtype=np.float32)
		Calib_P=np.zeros([self.max_objs, 3, 4], dtype=np.float32)
		Find_pcl=np.zeros([self.max_objs], dtype=np.bool8)
		...
		extra_kpts_depth_mask = np.zeros([self.max_objs, self.extra_kpts_num+10], dtype=np.float32) # whether the depths computed from three groups of keypoints are valid
		# 3d dimension
		...
		ori_mask=np.ones([self.max_objs],dtype=np.bool)
		...
		for i, obj in enumerate(objs):
			...
			extra_kpts_3D=obj.extra_kpts_3D
			extra_kpts_2D, _ =calib.project_rect_to_image(obj.generate_extra_kpts_3d_loc())
			find_pcl=obj.find_pcl
			...
			extra_kpts_x_visible = (extra_kpts_2D[:, 0] >= 0) & (extra_kpts_2D[:, 0] <= img_w - 1)
			extra_kpts_y_visible = (extra_kpts_2D[:, 1] >= 0) & (extra_kpts_2D[:, 1] <= img_h - 1)
			# extra_kpts_z_visible = extra_kpts_2D[:,2].astype(np.bool) #occlusion
			extra_kpts_z_visible = (obj.generate_extra_kpts_3d_loc()[:,-1]>0)
			# xyz visible
			keypoints_visible = keypoints_x_visible & keypoints_y_visible & keypoints_z_visible
			extra_kpts_visible = extra_kpts_x_visible & extra_kpts_y_visible & extra_kpts_z_visible
			...
			extra_kpts_depth_valid = extra_kpts_visible
			...
			extra_kpts_2D = (extra_kpts_2D[:,:2] + pad_size.reshape(1, 2).repeat(extra_kpts_2D.shape[0],axis=0)) / self.down_ratio
			...
			if (bbox_dim > 0).all() and (0 <= target_center[0] <= final_output_w - 1) and (0 <= target_center[1] <= final_output_h - 1):
				rot_y = obj.ry
				alpha = obj.alpha
				...
				extra_tmp = np.concatenate((extra_kpts_2D - target_center.reshape(1, -1), extra_kpts_visible[:, np.newaxis]), axis=1)
				EXTRA_KPTS_2D[i] = np.vstack((extra_tmp,keypoints[i]))
				EXTRA_KPTS_3D[i] = np.vstack((extra_kpts_3D,obj.raw_kpts_3d))
				Calib_P[i]=calib.P
				Find_pcl[i]=find_pcl
				keypoints_depth_mask[i] = keypoints_depth_valid
				extra_kpts_depth_mask[i] = np.concatenate((extra_kpts_depth_valid,keypoints_visible))
		...
		target.add_field("extra_kpts_2d", EXTRA_KPTS_2D)
		target.add_field("extra_kpts_3d", EXTRA_KPTS_3D)
		target.add_field('Calib_P',Calib_P)
		target.add_field("find_pcl", Find_pcl)
		target.add_field("extra_kpts_depth_mask", extra_kpts_depth_mask)
		...
		target.add_field('img_idx',original_idx)
		target.add_field('ori_mask',ori_mask)

...
def get_dataset(args):
    if args.gpu is not None:
        ...
        num_workers=args.workers, pin_memory=True, drop_last=True,sampler=val_sampler)
...
class GMW_data():
    ...
    def replace_location(self,new_loc,img_idx):
        ...
        for i in range(b):
            ...
            idx=int(img_idx[i][1])
            self.data[img_id][idx]['pred_location']=new_loc[i].cpu().numpy().tolist()
    ...
    def write_all_results(self):   
        ...
        if self.args.test_all:
            ...
            for img in imgs:
                ...
                f=open(os.path.join(self.result_dir,img+'.txt'),'w')
                f.close()
        for file_name in tqdm(self.data.keys()):
            results=self.data[file_name]
            for i, result in enumerate(results):
                ...
                if type(score) == list:
                    score=score[0]
                self.write_detection_results('Car', self.result_dir, file_name, box, dim, pos, ori, score)
...
def main():
    ...
    if args.evaluate:
        ...
        if args.local_rank==0 or args.gpu is not None:
            kitti_eval.write_all_results()
        return

    for epoch in range(args.start_epoch+1, args.epochs+1):
        if (epoch >= args.reg_loss_start_epoch):
            args.reg_weight=1.0
            args.cls_weight=0.1
        model.train()
        ...
        if epoch % args.val_freq==0 and epoch > 0 :
            ...
            if args.local_rank == 0 or args.gpu is not None:
                mAP_now,All_mAP=kitti_eval.write_all_results()
...
def validate(val_loader, model, epoch, args,kitti_eval):
    ...
    with torch.no_grad():
        ...
        for batch_index, (kpts_2d,kpts_3d,pred_rot,raw_location,img_idx) in enumerate(val_loader):
            if args.gpu is not None:
                kpts_2d = kpts_2d.cuda(args.gpu, non_blocking=True)
                kpts_3d = kpts_3d.cuda(args.gpu, non_blocking=True)
                pred_rot = pred_rot.cuda(args.gpu, non_blocking=True)
